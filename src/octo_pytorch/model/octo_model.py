...
from octo_pytorch.model.components.transformer import BlockTransformer, TokenGroup, PrefixGroup, TimestepGroup, AttentionRule
from octo_pytorch.model.components.image_tokenizer import SmallStem, regex_filter, generate_proper_pad_mask
...
class OctoModel(nn.Module):
    ...
    def __init__(self, repeat_task_tokens: bool = False):
        super().__init__()
        ...
        self.task_language_pos_embedding = nn.Parameter(
            torch.randn(1, 16, self.token_embedding_size) * 0.02
        )
        ...
        # Projections
        self.obs_primary_projection = nn.Linear(512, 768)
        self.obs_wrist_projection = nn.Linear(512, 768)
        self.task_language_projection = nn.Linear(768, 768)
    ...
    def forward(self, observations: Dict[str, torch.Tensor], 
                ...
                embodiment_action_dim: Optional[int] = None) -> torch.Tensor:
        ...
        for name, tokenizer in self.observation_tokenizers.items():
            if name in observations:
                ...
                mask = torch.logical_and(
                    timestep_pad_mask.unsqueeze(-1), token_group.mask
                )
        ...
        readout_tokens = torch.zeros(
            (batch_size, horizon, 1, self.token_embedding_size), 
            device=timestep_pad_mask.device
        )
        readout_tokens += self.readout_embedding[:, :horizon]
        readout_mask = torch.ones(
            (batch_size, horizon, 1), dtype=torch.bool, device=timestep_pad_mask.device
        )
        ...
        # Run transformer
        _, timestep_outputs = self.transformer(
            prefix_groups, 
            timestep_groups
        )
...
class OctoTransformer(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int, num_layers: int, dim_feedforward: int, max_horizon: int, repeat_task_tokens: bool = False):
        super().__init__()
        self.repeat_task_tokens = repeat_task_tokens
    ...
    def __init__(self, output_size: int, learnable: bool = True):
        ...
        if self.learnable:
            ...
            self.kernel = nn.Parameter(
                torch.normal(0, 0.2, size=(output_size // 2, 1))
            )
...
class DiffusionActionHead(nn.Module):
    ...
    def forward(self, 
                ...
                noisy_actions: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def predict_action(self,
                      ...
                      sample_shape: tuple = ()) -> torch.Tensor:
        ...
        flat_action_mask = action_mask.view(*sample_shape, batch_size, window_size, self.action_horizon * self.action_dim)
        
        # DDPM sampling loop
        def scan_fn(current_x, time):
            input_time = torch.full(
                (*current_x.shape[:-1], 1), time, device=device, dtype=torch.float32
            )
...
class ScoreActor(nn.Module):
    ...
    def __init__(self, out_dim: int, in_dim: int, time_dim: int, num_blocks: int, dropout_rate: float, hidden_dim: int, use_layer_norm: bool):
        super().__init__()
...
class MLPResNet(nn.Module):
    ...
    def __init__(self, num_blocks: int, out_dim: int, in_dim: int, dropout_rate: Optional[float] = None, use_layer_norm: bool = False, hidden_dim: int = 256, activation=nn.SiLU):
        super().__init__()
        self.num_blocks = num_blocks
        ...
        self.blocks = nn.ModuleList([
            MLPResNetBlock(
                hidden_dim, self.activation, dropout_rate, use_layer_norm
...
if __name__ == "__main__":
    ...
    # Create model
    model = OctoModel(repeat_task_tokens=True)
    ...
    tasks = {
        'language_instruction': model.text_processor.encode(sample_instructions)
    }
