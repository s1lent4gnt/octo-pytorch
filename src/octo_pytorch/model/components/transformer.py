...
class TokenGroup:
    ...
    def __post_init__(self):
        if self.mask.ndim != self.tokens.ndim - 1:
             raise ValueError(f"Mask must have one less dimension than tokens, but got {self.mask.ndim} and {self.tokens.ndim}")
...
class PrefixGroup(TokenGroup):
    ...
    def __post_init__(self):
        ...
        if len(self.tokens.shape) != 3:
            raise ValueError(f"PrefixGroup tokens must be (batch, n_tokens, d), but got shape {self.tokens.shape}")
...
class TimestepGroup(TokenGroup):
    ...
    def __post_init__(self):
        ...
        if len(self.tokens.shape) != 4:
            raise ValueError(f"TimestepGroup tokens must be (batch, horizon, n_tokens, d), but got shape {self.tokens.shape}")
        if len(self.mask.shape) != 3:
            raise ValueError(f"TimestepGroup mask must be (batch, horizon, n_tokens), but got shape {self.mask.shape}")


class MLPBlock(nn.Module):
    """Transformer MLP / feed-forward block."""
    
    def __init__(self, mlp_dim: int, dtype: torch.dtype = torch.float32, out_dim: Optional[int] = None, dropout_rate: float = 0.1):
        super().__init__()
...
class Encoder1DBlock(nn.Module):
    ...
    def __init__(self, mlp_dim: int, num_heads: int, d_model: int = 768, dtype: torch.dtype = torch.float32, dropout_rate: float = 0.1, attention_dropout_rate: float = 0.1):
        super().__init__()
        self.mlp_dim = mlp_dim
    ...
    def forward(self, inputs: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if attention_mask.dtype == torch.bool:
            attention_mask = attention_mask.float().masked_fill(~attention_mask, float('-inf')).masked_fill(attention_mask, 0.0)
...
class Transformer(nn.Module):
    ...
    def __init__(self, num_layers: int, mlp_dim: int, num_attention_heads: int, d_model: int = 768, dropout_rate: float = 0.1, attention_dropout_rate: float = 0.1, add_position_embedding: bool = False):
        super().__init__()
...
class BlockTransformer(nn.Module):
    ...
    def __init__(self, transformer_kwargs: Dict[str, Any], enforce_causal: bool = True, use_correct_attention: bool = False):
        super().__init__()
    ...
    def forward(self, prefix_groups: List[PrefixGroup], timestep_groups: List[TimestepGroup]) -> Tuple[List[PrefixGroup], List[TimestepGroup]]:
        ...
        prefix_outputs, timestep_outputs = self._split_output_tokens(
            output, prefix_groups, timestep_groups
        )
        
        return prefix_outputs, timestep_outputs
    
    def _assemble_input_tokens(self, prefix_groups: List[PrefixGroup], timestep_groups: List[TimestepGroup]) -> torch.Tensor:
        ...
        if len(prefix_groups) > 0:
            ...
            all_prefix_tokens = torch.zeros((batch_size, 0, token_dim), dtype=timestep_groups[0].tokens.dtype, device=timestep_groups[0].tokens.device)
    ...
    def _split_output_tokens(self, output_tokens: torch.Tensor, prefix_groups: List[PrefixGroup], timestep_groups: List[TimestepGroup]) -> Tuple[List[PrefixGroup], List[TimestepGroup]]:
        ...
        if len(prefix_groups) > 0:
            ...
            for group, embeddings in zip(prefix_groups, prefix_splits):
                all_prefix_outputs.append(PrefixGroup(
                    tokens=embeddings,
                    mask=group.mask,
                    name=group.name,
        ...
        for group, embeddings in zip(timestep_groups, timestep_splits):
            all_timestep_outputs.append(TimestepGroup(
                tokens=embeddings,
                mask=group.mask,
                name=group.name,
                attention_rules=group.attention_rules
    ...
    def _generate_attention_mask(self, prefix_groups: List[PrefixGroup], timestep_groups: List[TimestepGroup]) -> torch.Tensor:
    ...
    def _generate_pad_attention_mask(self, prefix_groups: List[PrefixGroup], timestep_groups: List[TimestepGroup]) -> torch.Tensor:
        ...
        if len(prefix_groups) > 0:
            ...
            prefix_pad_mask = torch.zeros((batch_size, 0), dtype=torch.bool,
