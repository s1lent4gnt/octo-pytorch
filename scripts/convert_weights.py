...
class OctoWeightConverter:
    def __init__(self, jax_params, pytorch_model):
    ...
    def _save_converted_params_info(self):
        ...
        import json
        with open("output/converted_params_info.json", "w") as f:
    ...
    def _convert_positional_embeddings(self):
        ...
        if obs_primary_pos is not None:
            self.pytorch_model.obs_primary_pos_embedding.data = torch.from_numpy(obs_primary_pos.copy()).float()
        ...
        if task_lang_pos is not None:
            self.pytorch_model.task_language_pos_embedding.data = torch.from_numpy(task_lang_pos.copy()).float()
    ...
    def _convert_projection_layers(self):
        ...
        if obs_primary_proj_kernel is not None:
            self.pytorch_model.obs_primary_projection.weight.data = torch.from_numpy(obs_primary_proj_kernel.T.copy()).float()
            self._track_converted_params(obs_primary_proj_kernel, kernel_name)
            if obs_primary_proj_bias is not None:
                self.pytorch_model.obs_primary_projection.bias.data = torch.from_numpy(obs_primary_proj_bias.copy()).float()
        ...
        if obs_wrist_proj_kernel is not None:
            self.pytorch_model.obs_wrist_projection.weight.data = torch.from_numpy(obs_wrist_proj_kernel.T.copy()).float()
            self._track_converted_params(obs_wrist_proj_kernel, kernel_name)
            if obs_wrist_proj_bias is not None:
                self.pytorch_model.obs_wrist_projection.bias.data = torch.from_numpy(obs_wrist_proj_bias.copy()).float()
        ...
        if task_lang_proj_kernel is not None:
            self.pytorch_model.task_language_projection.weight.data = torch.from_numpy(task_lang_proj_kernel.T.copy()).float()
            self._track_converted_params(task_lang_proj_kernel, kernel_name)
            if task_lang_proj_bias is not None:
                self.pytorch_model.task_language_projection.bias.data = torch.from_numpy(task_lang_proj_bias.copy()).float()
    ...
    def convert_transformer(self):
        # Access the encoder blocks through the correct path: PyTorchOctoTransformer -> BlockTransformer -> Transformer -> encoder_blocks
        try:
    ...
    def _convert_attention(self, jax_prefix, pytorch_layer):
        ...
        if q_kernel is not None and k_kernel is not None and v_kernel is not None:
            ...
            q_weight = q_kernel.reshape(768, 768)
            k_weight = k_kernel.reshape(768, 768)
            v_weight = v_kernel.reshape(768, 768)
        ...
        if out_kernel is not None:
            ...
            out_weight = out_kernel.transpose(2, 0, 1).reshape(768, 768)
    ...
    def _count_parameters(self):
        ...
        with open("output/jax_params_info.json", "w") as f:
        ...
        for path, param in self.jax_params.items():
            ...
            if any(skip in param_name.lower() for skip in [
        ...
        if self.total_jax_params == self.total_pytorch_params:
            ...
            print(f"⚠️  Total parameter count difference: {abs(self.total_jax_params - self.total_pytorch_params):,}")
            
        if jax_convertible_params == pytorch_convertible_params:
            print("✅ Convertible parameter counts match exactly!")
        else:
            print(f"⚠️  Convertible parameter count difference: {abs(jax_convertible_params - pytorch_convertible_params):,}")
    ...
    def _analyze_unconverted_params(self):
        ...
        for path, param in self.jax_params.items():
            ...
            elif any(component in param_name for component in [
                'pos_embedding', 'projection', 'MultiHeadDotProductAttention', 'LayerNorm',
                'diffusion_model/cond_encoder', 'diffusion_model/reverse_network', 'diffusion_model/time_preprocess'
            ]):
                converted = True
    ...
    def find_unconverted_params(self):
        ...
        # Track which parameters were actually converted during the conversion process
        converted_paths = set()
    ...
    def _convert_diffusion_action_head(self):
        ...
        if time_kernel is not None:
            try:
                pytorch_head.diffusion_model.time_preprocess.kernel.data = torch.from_numpy(time_kernel.copy()).float()
        ...
        if cond_dense0_kernel is not None:
            try:
                pytorch_head.diffusion_model.cond_encoder[0].weight.data = torch.from_numpy(cond_dense0_kernel.T.copy()).float()
                self._track_converted_params(cond_dense0_kernel, f"{jax_prefix}/cond_encoder/Dense_0/kernel")
                if cond_dense0_bias is not None:
                    pytorch_head.diffusion_model.cond_encoder[0].bias.data = torch.from_numpy(cond_dense0_bias.copy()).float()
        ...
        if cond_dense1_kernel is not None:
            try:
                pytorch_head.diffusion_model.cond_encoder[2].weight.data = torch.from_numpy(cond_dense1_kernel.T.copy()).float()
                self._track_converted_params(cond_dense1_kernel, f"{jax_prefix}/cond_encoder/Dense_1/kernel")
                if cond_dense1_bias is not None:
                    pytorch_head.diffusion_model.cond_encoder[2].bias.data = torch.from_numpy(cond_dense1_bias.copy()).float()
        ...
        # Diffusion network
        try:
            self._convert_diffusion_network(f"{jax_prefix}/reverse_network", pytorch_head.diffusion_model.reverse_network)
        except AttributeError:
    ...
    def _convert_mlp_resnet_block(self, jax_prefix, pytorch_module):
        ...
        if layernorm_scale is not None:
            try:
                ...
                self.conversion_log.append(f"Converted MLP ResNet block {jax_prefix.split('/')[-1]} LayerNorm")
            except AttributeError:
...
def main():
    ...
    torch.save(pytorch_model.state_dict(), "output/pytorch_octo_base_model.pth")
    print("\nSuccessfully converted and saved the PyTorch model to pytorch_octo_model.pth")
